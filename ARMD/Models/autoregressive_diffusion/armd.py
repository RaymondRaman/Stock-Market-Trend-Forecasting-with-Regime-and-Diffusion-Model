...
class ARMD(nn.Module):
    def __init__(
        ...
        self.model = Linear(n_feat=feature_size,
                            n_channel=seq_length, w_grad=w_grad, **kwargs)
    ...
    def model_predictions(self, x, t, clip_x_start=False, training=False):
        ...
        maybe_clip = partial(torch.clamp, min=-2,
                             max=2) if clip_x_start else identity
    ...
    def p_sample(self, x, t: int, clip_denoised=True):
        ...
        model_mean, _, model_log_variance, x_start = \
            self.p_mean_variance(x=x, t=batched_times,
        ...
        with torch.enable_grad():
            for i in range(K):
                ...
                if sigma.mean() == 0:
                    ...
                    infill_loss = (x_start[partial_mask] -
                                   tgt_embs[partial_mask]) ** 2
